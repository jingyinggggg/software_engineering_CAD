...
class DoctorViewStatus:
    ...
    def view(self, page: Page, params: Params, basket: Basket):
        ...
        def get_approved_booking():
            c = db.cursor()
            c.execute("SELECT appointmentDate, appointmentTime, clinic.name, appointmentStatus, bookingID FROM booking INNER JOIN clinic ON booking.clinicID = clinic.id WHERE "
                      "doctorID = ? AND proofStatus = ? ORDER BY bookingID DESC",
                      (user_id, 1,))
            record = c.fetchall()
            return record

        approved_booking = get_approved_booking()
        ...
        tab = Tabs(
            ...
            tabs=[
                Tab(
                    ...
                    content=Container(
                        width=340,
                        content=displayRequestedBooking(requested_booking)
                    ),
                ),
                Tab(
                    text="\t\t\tApproved\t\t\t",
                    content=Container(
                        width=340,
                        content=displayApprovedBooking(approved_booking)
                    ),
                ),
                Tab(
                    text="\t\t\tRejected\t\t\t",
                                               ...
                                               Container(
                                                   ...
                                                   content=Row(controls=[
                                                                   ...
                                                                   Container(padding=padding.only(left=55),
                                                                             content=Text("Prescription Status",
                                                                                          color="WHITE",
                                                                                          text_align=TextAlign.CENTER,
                                                                                          size=20,
                                                                                          font_family="RobotoSlab"
