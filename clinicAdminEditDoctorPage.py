...
class ClinicAdminEditDoctorPage:
    def __init__(self):
    ...
    def view(self, page: Page, params: Params, basket: Basket):
        ...
        success_dialog = AlertDialog(
            ...
            content=Text("You have updated the doctor details successfully!",
        ...
        def updateDatabase(e):
            ...
            if name.value != "" and email.value != "" and phoneNumber.value != "" and experience.value != "" and specialization.value != "" and description.value != "" and working_clinic.value != "" and working_time.value != "" and working_day.value != "" and non_working_day.value != "" and username.value != "" and password.value != "":
                c.execute(f"UPDATE doctors SET fullName = ?, username = ?, email = ?, password = ?, experience = ?, "
                          f"specialization = ?, description = ?, workingTime = ?, workingDay = ?, "
                          f"nonWorkingDay = ? WHERE id = {doctor_id}",
                          (name.value, username.value, email.value, password.value, experience.value, specialization.value, description.value, working_time.value, working_day.value, non_working_day.value))
                db.commit()
        ...
        return View(
            "/admin/clinicAdminEditDoctorPage/:user_id:doctor_id",
            controls=[
                Container(
                    width=350,
                    height=700,
